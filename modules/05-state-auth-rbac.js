...
var DEFAULT_PERMISSIONS = {
  ...
  production_manager: {sales:'view',jobs:true,factory:true,accounts:'view',service:false,
    pages:['dashboard','contacts',
      ...
      'factorydash','prodqueue','prodboard','factorybom','factorycap','factorydispatch','factoryaudit',
      'accdash','accbills','accweekly']},
  production_staff:   {sales:false,jobs:'view',factory:true,accounts:false,service:false,
    pages:['factorydash','prodqueue','prodboard','factorybom','factorycap','factorydispatch','factoryaudit',
      'jobdashboard','jobs']},
...
// Always returns a non-empty list and always contains the seeded admin so
// you can recover even if Supabase sync wiped the cache or the admin row was
// accidentally deactivated. Without this guard, an empty/corrupt
// `spartan_users` would lock everyone out with no in-app recovery path.
function getUsers(){
  var list = [];
  try {
    var s = localStorage.getItem('spartan_users');
    if (s) list = JSON.parse(s) || [];
    if (!Array.isArray(list)) list = [];
  } catch(e) { list = []; }
  // Merge in any DEFAULT_USERS entry whose email isn't already present.
  // Keeps the live list authoritative for users that exist, while ensuring
  // the seeded admin is always reachable.
  var byEmail = {};
  list.forEach(function(u){ if (u && u.email) byEmail[u.email.toLowerCase()] = u; });
  var seeded = false;
  DEFAULT_USERS.forEach(function(d){
    if (!byEmail[d.email.toLowerCase()]) { list.push(Object.assign({}, d)); seeded = true; }
  });
  if (seeded || list.length === 0) {
    try { localStorage.setItem('spartan_users', JSON.stringify(list)); } catch(e) {}
  }
  return list;
}
...
// Reachable from the "Emergency reset" link on the login screen. Clears the
// local user cache so getUsers() re-seeds DEFAULT_USERS on reload, restoring
// the seeded admin (admin@spartandoubleglazing.com.au / spartan2026). Other
// users will repopulate from Supabase on the next sync.
function emergencyResetLogin(){
  if (!window.confirm('Reset login cache?\n\nThis re-seeds the default admin so you can sign in again. Other users will repopulate from Supabase. Your data is not affected.')) return;
  try { localStorage.removeItem('spartan_users'); } catch(e) {}
  try { localStorage.removeItem('spartan_current_user'); } catch(e) {}
  location.reload();
}
window.emergencyResetLogin = emergencyResetLogin;
...
var users=getUsers();
var u=users.find(function(x){return x.email.toLowerCase()===e&&x.pw===p&&x.active;});
if(!u){
  var el=document.getElementById('loginErr');
  // Disambiguate the failure mode so users know what to fix.
  var byEmail=users.find(function(x){return x.email.toLowerCase()===e;});
  if(!e||!p) el.textContent='Enter both email and password.';
  else if(byEmail&&!byEmail.active) el.textContent='Account deactivated. Ask an admin to re-enable it.';
  else if(byEmail) el.textContent='Wrong password.';
  else el.textContent='No account with that email. Check the spelling or use Sign in with Google.';
